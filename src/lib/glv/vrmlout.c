...
int vrml_file_new_object(vrml_file_p vrml_file, int object_type,
  ...
  vrml_file->object = (vrml_object_p)realloc(vrml_file->object,
    vrml_file->num_objects * sizeof(vrml_object_t));
  ...
  if(object_type == VRML_POINTSET) {
    ...
    if(object_type == VRML_3NORMFACESET) {
      ...
      vrml_file->object[i].normal_fp = carmen_fopen(
        vrml_file->object[i].normal_filename, "w");
...
void vrml_add_point(vrml_file_p vrml_file, int num,
...
void vrml_add_line(vrml_file_p vrml_file, int num,
