...
static char *default_list[] = {"carmen.ini", "../carmen.ini",
  "../src/carmen.ini", "/etc/carmen/carmen.ini", 0};
...
static char **param_filenames = NULL;
...
find_valid_robots(char **robot_names, int *num_robots, int max_robots)
{
  ...
  while (param_filenames[fcount]) {
    fp = fopen(param_filenames[fcount], "r");
    if(fp == NULL)
      return -1;
      ...
      if(err != NULL) {
        ...
        if(left != NULL && right != NULL && left < right &&
          ...
          if(*num_robots > max_robots)
            carmen_die("Error: exceeded maximum number of robots in "
            "parameter files (%d).\n",
            max_robots);
...
read_parameters_from_file(void)
{
      ...
      if (mark) {
        if (strlen(mark) > MAX_VARIABLE_LENGTH-1) {
          carmen_warn("Bad file format of %s on line %d.\n"
            ...
            "than %u characters.\nSkipping this line.\n",
            param_filenames[fcount], count, mark, (int) strlen(mark),
            MAX_VARIABLE_LENGTH-1);
          continue;
        }
        strcpy(rvalue, mark);
  ...
  if (selected_robot && !found_desired_robot) {
    char param_filenames_str[4096];
    param_filenames_str[0] = '\0';
    fcount = 0;
    while (param_filenames[fcount]) {
      char param_filename_str[1024];
      sprintf(param_filename_str, "* %s\n", param_filenames[fcount]);
      strcat(param_filenames_str, param_filename_str);
      ++fcount;
    }

    carmen_die("Did not find a match for robot %s.\n"
      "Do you have the right init files?\n"
...
#ifndef COMPILE_WITHOUT_MAP_SUPPORT
  fprintf(stderr, "usage: %s [-ahr] [map_filename] [ini filenames] \n\n",
    progname);
#else
  fprintf(stderr, "usage: %s [-ahr] [ini filenames] \n\n",
    progname);
#endif
...
read_commandline(int argc, char **argv)
{
  ...
  for (cur_arg = optind; cur_arg < argc; cur_arg++) {
    ...
    if (binary > 0)
      ...
      argv[cur_arg]);
    param_filenames = (char **)realloc(param_filenames,
      (fcount+2)*sizeof(char *));
    carmen_test_alloc(param_filenames);
    param_filenames[fcount] =
      (char *)calloc(strlen(argv[cur_arg])+1, sizeof(char));
    param_filenames[fcount+1] = 0;
    carmen_test_alloc(param_filenames[fcount]);
    strcpy(param_filenames[fcount], argv[cur_arg]);
    ++fcount;
  }
  ...
  if (!param_filenames) {
    for (index = 0; default_list[index]; index++) {
      if (carmen_file_exists(default_list[index])) {
        param_filenames = (char **)realloc(param_filenames, 2*sizeof(char *));
        carmen_test_alloc(param_filenames);
        param_filenames[0] = default_list[index];
        param_filenames[1] = 0;
        break;
      }
    }
  ...
  while (param_filenames[fcount]) {
    char param_filename_str[1024];
    sprintf(param_filename_str, "* [31;1m%s[0m\n", param_filenames[fcount]);
    strcat(param_filenames_str, param_filename_str);
    ++fcount;
  }

  if (num_robots == 0) {
    carmen_warn("Loading parameters for default robot using param files\n%s",
      param_filenames_str);
    return 0;
  }
  ...
  if (selected_robot == NULL && num_robots > 1)
    usage(argv[0], "The ini_files contain %d robot definitions.\n"
  ...
  carmen_warn("Loading parameters for robot [31;1m%s[0m using param "
    "files\n%s", selected_robot, param_filenames_str);
        ...
        void *clientData __attribute__ ((unused)))
{
  ...
  if (alphabetize)
    ...
    for (variable_count = 0; variable_count < num_variables;
      variable_count++)
    {
      param_index = lookup_parameter(query.module_name,
